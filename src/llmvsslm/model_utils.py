...
DEFAULT_MODEL_BASE_DIR = os.environ.get("LMVSSLM_MODEL_DIR", str(REPO_ROOT / "outputs" / "models"))
...
def get_fast_language_model():
    try:
        ...
        raise ImportError(
            "Llama models require optional LLM dependencies. "
...
class ModelManager:
    ...
    def __init__(self, fixed_model_names: list[str], model_base_dir=DEFAULT_MODEL_BASE_DIR, default_model_name='ensemble/llama_1b_model'):
        ...
        if not model_base_path.is_absolute():
            model_base_path = REPO_ROOT / model_base_path
        self.model_base_dir = str(model_base_path.resolve())
        ...
        self.current_model_path = os.path.join(self.model_base_dir, 'mathBERT')
    ...
    def load_model(self, model_name):
        ...
        load_path = os.path.join(self.model_base_dir, model_name)
        ...
        try:
            if model_name == 'mathBERT':
                ...
                model, tokenizer = get_fast_language_model().from_pretrained(
                ...
                for model_key in models_list:
                    ...
                    try:
                        ...
                        if model_key == 'ensemble/llama_1b_model':
                            ...
                            t5_model_dir = component_path
                            comp_tokenizer = AutoTokenizer.from_pretrained(t5_model_dir)
                            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                            comp_model = AutoModelForSeq2SeqLM.from_pretrained(t5_model_dir).to(device)
                            comp_model.eval()
                        else:
...
def llama_predict(text, model, tokenizer):
    get_fast_language_model().for_inference(model)
